    ...
    <Drawer.Navigator id="LeftDrawer" drawerContent={(props, rootNavigator) => <DrawerContent {...props} rootNavigator={rootNavigator}/>} screenOptions={({navigation})=>(
      {
        ...
        {following.map((sport) => (
          <Drawer.Screen options={{drawerItemStyle: [drawerStyles.drawerItems, drawerStyles.gray], drawerIcon: ()=> (<FontAwesome name={"star-o"} size={34} color={'#FFCC00'} />)}} name={`${sport.sport} Tab`} component={SportTab} initialParams={{sport}}/>
        ))}
    </Drawer.Navigator>
  );
}
